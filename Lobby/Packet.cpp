...
bool ReqUserInfo::Handler(std::shared_ptr<Server::GameSession> session, const Payload& payload) {
    ...
    session->Send(
        ResUserInfo(
            true, std::to_string(user_info->uid()), user_info->name(), user_info->avatar_path(), 
...
) : success_(success), uid_(uid), name_(name), avatar_path_(avatar_path), money_(money) {
}
